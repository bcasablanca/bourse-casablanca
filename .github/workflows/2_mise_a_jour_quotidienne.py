...
def main():
    ...
    # ── Regeneration HTML ─────────────────────────────────────
    gen = DOSSIER / "3_generer_html.py"
    if gen.exists():
        info("Regeneration du dashboard HTML ...")
        import subprocess as sp
        r = sp.run([sys.executable, str(gen)],
                   capture_output=True, text=True, timeout=180, cwd=str(DOSSIER))
        if r.returncode == 0: ok("Dashboard HTML regenere.")
        else: warn("Regeneration HTML : " + (r.stderr[-300:] if r.stderr else "erreur"))
